<script lang="ts">
	...
	async function handleDiscordLogin() {
		...
		try {
			...
			error = e instanceof Error ? e.message : 'Discord login failed';
			loading = false;
		}
	}
...
<svelte:head>
	<title>Login - BaconAlgo</title>
	<meta name="description" content="Login to your BaconAlgo account" />
</svelte:head>

<div class="min-h-screen flex items-center justify-center px-4 py-12">
	<!-- Background decoration -->
	<div class="absolute inset-0 overflow-hidden pointer-events-none">
		<div class="absolute top-1/4 left-1/4 w-96 h-96 bg-bacon-orange/10 rounded-full blur-3xl"></div>
		<div class="absolute bottom-1/4 right-1/4 w-96 h-96 bg-bacon-red/10 rounded-full blur-3xl"></div>
	</div>
	...
	<div class="w-full max-w-md relative z-10">
		<GlassCard padding="xl">
			...
			<div class="text-center mb-8">
				...
				<p class="text-text-secondary">Sign in to your BaconAlgo account</p>
			</div>
			...
			<!-- Email/Password Form -->
			<form onsubmit={(e) => { e.preventDefault(); handleEmailLogin(); }} class="space-y-4 mb-6">
				<div>
				...
				<div class="flex items-center justify-between">
					<label class="flex items-center">
						<input
							...
							class="w-4 h-4 rounded border-white/10 bg-white/5 text-bacon-orange focus:ring-bacon-orange/50 focus:ring-offset-0"
						/>
						<span class="ml-2 text-text-secondary text-sm">Remember me</span>
					</label>
					<a href="/forgot-password" class="text-bacon-orange text-sm hover:underline">
						Forgot password?
					</a>
				</div>
				...
				{#if show2FA}
					<div class="animate-fadeIn">
						<label for="2fa" class="block text-text-primary text-sm font-semibold mb-2">
							2FA Code
						</label>
						<input
							id="2fa"
							...
							placeholder="000000"
							maxlength="6"
							class="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white text-center text-lg tracking-widest placeholder-text-secondary focus:outline-none focus:ring-2 focus:ring-bacon-orange/50 transition-all"
						/>
					</div>
				{/if}
			...
			<div class="text-center mb-6">
				<div class="inline-block p-4 bg-white/5 border border-white/10 rounded-lg">
					...
					<p class="text-text-secondary text-xs mt-2">Scan to login on mobile</p>
				</div>
			</div>
			...
			<div class="text-center">
				<p class="text-text-secondary text-sm">
					...
					<a href="/register" class="text-bacon-orange hover:underline font-semibold">
						Register
					</a>
				</p>
			</div>
